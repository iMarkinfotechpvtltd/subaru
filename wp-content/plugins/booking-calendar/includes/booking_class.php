...
class wpdevart_bc_BookingCalendar {
	...
	public function __construct($date = '', $id, $theme_option, $calendar_data, $form_option, $extra_field, $selected = array(),$ajax = false,$widget=false,$text_for=array()) {
		...
		if(isset($this->theme_option["default_month"]) && $this->theme_option["default_month"] != 0 && $ajax == false) {
			$date_m = substr_replace($date,$this->theme_option["default_month"],5,2);
			if(strtotime( $date ) < strtotime( $date_m )) {
				$date = $date_m;
			}
		}
		if(isset($this->theme_option["default_year"]) && $this->theme_option["default_year"] != ""&& $ajax == false) {
	...
	public function save_reserv($data,$submit){
		...
		/*count item*/
		if($check_in) {
			$item_count = abs($this->get_date_diff($check_in,$check_out)) + 1;
		} else {
			$item_count = 1;
		}
